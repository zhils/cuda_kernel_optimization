...
**B=8, L=2048, D=512, H=256 性能：** 87.5 ms
...
**B=8, L=2048, D=512, H=256 性能：** 563.6 ms（**比 v0 还慢 6.4x**）
...
**B=8, L=2048, D=512, H=256 性能：** 29.4 ms（**比 v0 快 3.0x，比 v1 快 19x**）

| 规模 | v0 | v1 | **v2** | v2 vs v0 |
|------|:--:|:--:|:-----:|:--------:|
| B=1, L=128, D=64, H=32 | 0.042 ms | 1.223 ms | **0.016 ms** | **2.6x** |
| B=4, L=1024, D=512, H=256 | 24.95 ms | 282.2 ms | **7.58 ms** | **3.3x** |
| B=8, L=2048, D=512, H=256 | 87.5 ms | 563.6 ms | **29.4 ms** | **3.0x** |
...
- 在 D 较大的场景下（D=512），SMEM tiling 优势更为明显

**B=8, L=2048, D=512, H=256 性能：** 28.6 ms（**比 v0 快 3.1x，比 v2 快 3%**）

| 规模 | v0 | v2 | **v3** | v3 vs v2 |
|------|:--:|:--:|:-----:|:--------:|
| B=1, L=128, D=64, H=32 | 0.042 ms | 0.016 ms | **0.013 ms** | **-19%** |
| B=4, L=1024, D=512, H=256 | 24.95 ms | 7.58 ms | **7.51 ms** | -1% |
| B=8, L=2048, D=512, H=256 | 87.5 ms | 29.4 ms | **28.6 ms** | **-3%** |

---

## 4. 完整性能对比

测试环境：RTX 3080 Ti（sm_86, 12GB），CUDA 12.8，B=8, L=2048, D=512, H=256, k_size=4。

| 版本 | 耗时(ms) | 加速比(v0) | 关键思想 | 中间缓冲数 |
|:----|:--------:|:----------:|----------|:---------:|
| v0 | 87.5 | 1.0x | 5 个分离 kernel | 7 |
| v1 | 563.6 | **0.16x** ❌ | 单 kernel 融合，串行 L | 0 |
| v2 | 29.4 | **3.0x** 🚀 | 双 kernel + 扁平 grid + vec4 | 1 |
| v3 | 28.6 | **3.1x** 🚀 | 双 kernel + 2D grid + SMEM | 1 |

### 各规模吞吐量（GFLOPS）

| 规模 | v0 | v2 | v3 |
|------|:--:|:--:|:--:|
| B=1, L=128, D=64, H=32 | 1.79 | **21.7** | **31.1** |
| B=1, L=256, D=128, H=64 | 5.59 | **112.8** | **106.8** |
| B=2, L=512, D=256, H=128 | 48.3 | **839.1** | **671.9** |
| B=4, L=1024, D=512, H=256 | 71.0 | **1,227.5** | **1,244.9** |
| B=8, L=2048, D=512, H=256 | 148.9 | **1,230.9** | **1,267.3** |

吞吐量计算方式：统计 9 步操作的总 FLOPs（4 次 D×H 的线性投影 + 3 次 k_size×H 的因果卷积 + 3 次 H 的 SiLU + 1 次 H 的门控乘法），除以实测耗时。
...
1. **恢复满并行度**（v2 扁平 grid）：**~19x** — 最关键的决策，决定 GPU 能否充分调度
2. **消除中间缓冲**（v2 双 kernel）：**~3x** — 减少带宽压力
3. **float4 向量化加载**（v2）：**~20-30%** — 减少指令数，提高带宽利用率
4. **SMEM tiling**（v3）：**~3%** — 当前场景下 x 非主要瓶颈
