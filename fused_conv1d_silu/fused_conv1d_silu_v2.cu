...
// Fused Conv1D + SiLU v2: 双 kernel 融合，将 linear_qkv + linear_z + split 合并为 Kernel A，
// causal_conv + SiLU + gate_mul 合并为 Kernel B。与 v1 区别在于恢复到 (b,t,h) 全并行，
// 仅保留一个中间缓冲 z_proj，主路径使用 float4 访存。
