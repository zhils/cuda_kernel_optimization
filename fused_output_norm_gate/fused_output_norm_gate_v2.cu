...
// ============================================================================
// Fused Output Norm Gate v2: Fully fused single kernel, no intermediates
// ============================================================================
// v2 fuses ALL 4 steps into one kernel:
//   Gate_Proj + SiLU → RMSNorm → Multiply_Gate → Linear_Output_Proj
//
// Key optimizations vs v0/v1:
//   1. Single kernel launch (v0 has 4, v1 has 2)
//   2. Zero intermediate global buffers: gate, x_hat, y all live in SMEM
//   3. SMEM-based y vector shared across threads for output projection
//   4. No global memory traffic between steps — all in SMEM/registers
// ============================================================================
...
// ----------------------------------------------------------------------------
// Fully fused kernel: all 4 steps in one
// grid(B, L), block(128), shared_mem = gate[H] + y[H]
// ----------------------------------------------------------------------------
__global__ void FusedOutputNormGateV2Kernel(
  ...
  float* s_gate = smem;  // H floats
  float* s_y = smem + H; // H floats
  ...
  // ==================================================================
  // Step 1: Gate Projection + SiLU → s_gate[H]
  ...
  // ==================================================================
  // Step 2: RMSNorm — warp shuffle reduction across all threads
  // ==================================================================
  #pragma unroll
  ...
  float rcp_rms = 1.0f / sqrtf(s_warp_sum[0] / static_cast<float>(H) + kEps);

  // ==================================================================
  // Step 3: Normalize + Multiply Gate → s_y[H]
  // ==================================================================
  for (int h = tid; h < H; h += blockDim.x) {
    float val = s_gate[h];
    float x_hat = val * rcp_rms * g[h];
    s_y[h] = x_hat * val;
  }
  __syncthreads();

  // ==================================================================
  // Step 4: Output Projection — each thread handles its D_out values
  // Reads s_y[H] from SMEM, W_out from global, writes to global
  // ==================================================================
  int H4 = H / 4;
  ...
  for (int d = tid; d < D_out; d += blockDim.x) {
    ...
    for (int i = 0; i < H4; ++i) {
      float4 yv = *reinterpret_cast<const float4*>(s_y + i * 4);
...
static void RunGpuV2(const float* d_x,
                     ...
                     int B, int L, int D_in, int H, int D_out) {
  int threads = 128;
  size_t smem_size = 2 * H * sizeof(float);  // gate[H] + y[H]

  dim3 grid(B, L);
  FusedOutputNormGateV2Kernel<<<grid, threads, smem_size>>>(
...
int main() {
  ...
  std::cout << "=== Fused Output Norm Gate V2 (Fully Fused, No Intermediates) ===\n";
  ...
  for (const auto& tc : test_cases) {
    ...
    // Device memory — only need x, W, bias, and output!
