...
static void RunGpuV0(const float* d_Q, const float* d_K,
  ...
  L2NormKernel<<<grid_q, block>>>(d_Q, d_Q_hat, B, N_q, H_q);
  L2NormKernel<<<grid_k, block>>>(d_K, d_K_hat, B, N_k, H_k);
}
...
int main() {
  ...
  // Test configurations: (B, N_q, H_q, N_k, H_k)
  ...
  std::ofstream ofs("data/results/fused_l2_norm_qk_v0_results.csv");
  ofs << "B,N_q,H_q,N_k,H_k,cpu_ms,gpu_ms,speedup,max_abs_diff_q,max_abs_diff_k,check\n";

  std::cout << "=== Fused L2 Norm Q/K V0 (Naive Separate Kernels) ===\n";
  ...
  std::cout << "\nResults saved to data/results/fused_l2_norm_qk_v0_results.csv\n";
  return 0;
}
