...
> "判断算子能否融合，核心是三个条件：
> 
> 第一，数据依赖链——前一个算子的输出恰好是后一个的输入。
> 第二，中间结果不被其他分支使用——否则融合后它拿到了。
> 第三，融合后的资源不超限——smem 不超过 48KB，寄存器每个线程不超过 255 个。
> 
> 评估融合收益，我通常先算内存流量。比如 fused_output_norm_gate 的 4 步，融合前总共读写 160MB，融合后只需要 48MB，节省 70% 的带宽。
> 
> 但也要注意融合不会减少计算量，所以实际 speedup 取决于带宽节省和 launch overhead 消除的叠加效果。通常 2-3 个连续 element-wise 操作的融合，speedup 在 1.5-2x。"
