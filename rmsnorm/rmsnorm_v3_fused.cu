// RMSNorm V3: Fused kernel — vectorized + parallel output + fast math
//
// Key optimizations over V2:
//   - All threads participate in BOTH reduction AND output writing
//   - __fmul_rn intrinsics for deterministic rounding
//   - Cross-warp reduction via shared memory
//   - Maximizes memory bandwidth utilization for this memory-bound kernel
...
__global__ void RMSNormFusedKernel(const float* __restrict__ x,
    ...
    for (int off = WARP_SIZE / 2; off > 0; off >>= 1)
        sq_sum += __shfl_down_sync(0xffffffff, sq_sum, off);
    ...
    for (int c = tid; c < n4; c += blockDim.x) {
        float4 val4 = x4[c];
        float4 res4;
        res4.x = __fmul_rn(__fmul_rn(val4.x, rms), weight[c * 4]);
        res4.y = __fmul_rn(__fmul_rn(val4.y, rms), weight[c * 4 + 1]);
        res4.z = __fmul_rn(__fmul_rn(val4.z, rms), weight[c * 4 + 2]);
        res4.w = __fmul_rn(__fmul_rn(val4.w, rms), weight[c * 4 + 3]);
        y4[c] = res4;
    }

    for (int c = n4 * 4 + tid; c < cols; c += blockDim.x)
        y[offset + c] = __fmul_rn(__fmul_rn(x[offset + c], rms), weight[c]);
}
...
int main() {
    ...
    for (size_t i = 0; i < cases.size(); ++i) {
        ...
        int threads = 256;
        RMSNormFusedKernel<<<rows, threads>>>(dx, dy, dw, rows, cols, kEps);
